...
if (isset($_POST['action']) && $_POST['action'] == 'ajout' )
{
	...
	if (isset($_FILES['image']) && $_FILES['image']['type'] != '') {
		if ($_FILES['image']['type']=="image/jpeg" || $_FILES['image']['type']=="image/png" || $_FILES['image']['type']=="image/gif" || $_FILES['image']['type']=="image/webp" ) {
			$base_name = $ids."-image-" . pathinfo($_FILES['image']['name'], PATHINFO_FILENAME);
			$base_name = str_replace([' ', 'é', 'è', 'à', 'ù', 'ç'], ['-', 'e', 'e', 'a', 'u', 'c'], $base_name);
			$dest_base = "../media/applications/" . $base_name;
			
			$webp_filename = convertAndSaveWebP($_FILES['image']['tmp_name'], $dest_base);
			if ($webp_filename) {
				$image = $webp_filename;
			} else {
				$destination = $base_name . "." . pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
